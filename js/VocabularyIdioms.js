...
function updateContent(language,divContent){
    ...
    if(getParam('all') != -1) {
        ...
        if($.cookie('searchIdioms') == null || $.cookie('searchIdioms') == 'null' || $.cookie('searchIdioms') == undefined){
...
function contentAll(language,divContent) {
    ...
    $.ajax({
        ...
        success: function (data) {
            divContent.innerHTML += contentList(language,JSON.parse(data),'idioms?all&');
        }
    });
}
...
function contentLiked(language,divContent) {
    if (isAuthorized()) {
        ...
        $.ajax({
            ...
            success: function (data) {
                divContent.innerHTML += contentList(language, JSON.parse(data), 'idioms?liked&');
            }
        });
    } else {
        defaultTab(language, divContent);
    ...
    if(isAuthorized()) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                divContent.innerHTML += contentList(language,items,'idioms?recommended&');
            }
        });
    } else {
        defaultTab(language,divContent);
...
function contentSearch(language,divContent) {
    ...
    $.ajax({
        ...
        success: function (data) {
            divContent.innerHTML += contentList(language,JSON.parse(data),'idioms?search&');
        }
    });
...
function defaultTab(language,divContent) {
    document.location.hash = '#idioms?all&page=1';
...
function contentTabs(language,divContent,activeTab) {
    if(activeTab == 1){
        ...
        divContent.innerHTML += '<input onclick="document.location.hash = \'#idioms?all&page=1\'; updateContent($.cookie(\'language\'),document.getElementById(\'div-content\'));" type="button" class="submenu-button" value="'+titles[language]["button-all"]+'">';
    }
    if(isAuthorized()){
        if(activeTab == 2) {
            divContent.innerHTML += '<input type="button" class="submenu-button-active" value="' + titles[language]["button-liked"] + '">';
        } else {
            divContent.innerHTML += '<input onclick="document.location.hash = \'#idioms?liked&page=1\'; updateContent($.cookie(\'language\'),document.getElementById(\'div-content\'));" type="button" class="submenu-button" value="' + titles[language]["button-liked"] + '">';
        }
        if(activeTab == 3) {
            divContent.innerHTML += '<input type="button" class="submenu-button-active" value="' + titles[language]["button-recommended"] + '">';
        } else {
            divContent.innerHTML += '<input onclick="document.location.hash = \'#idioms?recommended&page=1\'; updateContent($.cookie(\'language\'),document.getElementById(\'div-content\'));" type="button" class="submenu-button" value="' + titles[language]["button-recommended"] + '">';
        }
    }
    if(activeTab == 4){
        divContent.innerHTML += '<input type="button" class="submenu-button-active" value="'+titles[language]["button-search"]+'"></br>';
    } else {
        divContent.innerHTML += '<input onclick="document.location.hash = \'#idioms?search&page=1\'; updateContent($.cookie(\'language\'),document.getElementById(\'div-content\'));" type="button" class="submenu-button" value="'+titles[language]["button-search"]+'"></br>';
    }
}
...
function contentList(language,items,link) {
    ...
    if(items.length>1){
        ...
        for (var i=0; i<items.length-1; i++) {
            ...
            '<td><p class="list-text" onclick="document.location.hash = \'#idioms?id=' + items[i][0] + '\'; updateContent($.cookie(\'language\'),document.getElementById(\'div-content\'));">' + items[i][1] + ' - ' + items[i][2] + '</p></td>' +
            '</tr>';
        }
        cont += '</table>';
